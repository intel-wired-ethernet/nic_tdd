...
/** @brief Unique descriptor for the PCI_IOV_ATTACH_NAME() method */
extern struct kobjop_desc pci_iov_attach_name_desc;
/** @brief A function implementing the PCI_IOV_ATTACH_NAME() method */
typedef int pci_iov_attach_name_t(device_t dev, device_t child,
                                  struct nvlist *pf_schema,
                                  struct nvlist *vf_schema, const char *name);

static __inline int PCI_IOV_ATTACH_NAME(device_t dev, device_t child,
                                        struct nvlist *pf_schema,
                                        struct nvlist *vf_schema,
                                        const char *name)
{
	kobjop_t _m;
	KOBJOPLOOKUP(((kobj_t)dev)->ops,pci_iov_attach_name);
	return ((pci_iov_attach_name_t *) _m)(dev, child, pf_schema, vf_schema, name);
}
